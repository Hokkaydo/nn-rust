...
pub fn gradient_descent(learning_rate: f32) -> Box<dyn Fn(Vec<&Tensor>) -> Vec<Tensor>> {
...
pub fn adam(
    ...
    Box::new(move |values: Vec<&Tensor>| {
        ...
        let m = if values.len() > 2 {
            ...
            Tensor::new(vec![0.0; val.data.len()], val.shape.clone())
        };
        ...
        let m_hat = &m / (1.0 - beta1.powi(t.data[0] as i32));
        let v_hat = &v / (1.0 - beta2.powi(t.data[0] as i32));
